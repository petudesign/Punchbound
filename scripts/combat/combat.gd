...
func _physics_process(delta: float) -> void:
	...
	if health <= 0 or not attacking:
		return
	elapsed += delta
	if elapsed >= attack.startup and elapsed < attack.startup + attack.active:
		...
		for result in get_world_2d().direct_space_state.intersect_shape(_query):
			...
			if target_id not in _hit_targets:
				...
				if target.receive_hit(attack, attack_facing):
					freeze_remaining = attack.hitstop
					landed_hit.emit()
	if elapsed >= attack.startup + attack.active + attack.recovery:
		attacking = false
...
func receive_hit(data: Resource, direction: float) -> bool:
	...
	_flash = 0.16
	get_parent().velocity = Vector2(direction * data.knockback, -85.0)
...
func _attack_center() -> Vector2:
	...
	var elevation: float = 18.0 if crouching and not enemy else attack.elevation
	return Vector2(attack_facing * (19.0 + attack.reach / 2.0), -elevation)
...
func _draw() -> void:
	...
	if attacking:
		...
		elif elapsed < attack.startup + attack.active:
